...
if (isset($_POST['ordersubmit'])) {
	if (strlen($_SESSION['login']) == 0) {
		...
		$totalAmount = $_SESSION['tp'];

		$insertOrderQuery = "INSERT INTO orders (userId, total) VALUES ('$userId', '$totalAmount')";

		if (mysqli_query($con, $insertOrderQuery)) {
			// Lấy id của đơn hàng vừa được tạo
			$orderId = mysqli_insert_id($con);

			// Bước 2: Duyệt qua giỏ hàng và thêm từng sản phẩm vào orderdetails
			foreach ($value as $qty => $val34) {
				$insertOrderDetailQuery = "INSERT INTO orderdetails (orderId, productId, quantity) VALUES ('$orderId', '$qty', '$val34')";
				mysqli_query($con, $insertOrderDetailQuery);
			}

			// Hiển thị thông báo và chuyển hướng tới trang xác nhận
			echo "<script>alert('Order has been placed successfully');</script>";
			unset($_SESSION['cart']); // Xóa giỏ hàng sau khi tạo đơn hàng
			header('location:index.php');
		} else {
			echo "<script>alert('There was an error placing the order');</script>";
		}
